#!/bin/sh
#
# Add/remove aliases from /etc/hosts. Add "sed * /etc/hosts"  to the sudoers config
...
do_list () {
	sed -e '/\blocalhost\b/d' "$HOSTS_FILE"
}
...
do_remove() {
	...
	grep -E "[[:space:]]$name([[:space:]]|$)" "$HOSTS_FILE" |while read -r line; do
		...
		sudo sed -i -e "/$line/d" "$HOSTS_FILE"
	done
}
